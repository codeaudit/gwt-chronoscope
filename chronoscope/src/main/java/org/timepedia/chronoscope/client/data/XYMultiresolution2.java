...
/**
 * This class implements general purpose code to pre-process an XY Dataset into
 * multiresolution representation allowing the choice of strategies for
 * partitioning the domain, and computing functions on range values with the
 * partition. <p/> For example, a domain may be partitioned into weeks, months,
 * quarters, years, etc and for each partition, you may apply sum, avg, max, etc
 * to compute a representative value for each partition.
 */
public class XYMultiresolution2 {
  ...
  private void compute(XYPartitionStrategy strategy,
      XYAggregateFunction function) {
    ...
    for (int level = 0; level < multiDomain.length; level++) {
      if (level == 0) {
        ...
        XYPartitioner partitioner = strategy.getPartitioner(this, level
            + startLevel - 1);
        int numSamples = partitioner.getNumPartitions(this, intervalStart,
            intervalEnd);
        previousIndices[level] = new int[numSamples];

        System.out.println("Got " + numSamples + " for partitioner "
            + partitioner);
        ...
        int startIndex = 0;
        partitioner.getPartition(currentPartition, this, intervalStart,
            intervalEnd, part);
        while (index < domain.length && currentPartition < numSamples) {
          if (domain[index] >= part.partitionStart
              && domain[index] <= part.partitionEnd && ++index < domain.length) {
          } else {
            System.out.println("Start index " + startIndex + " end index "
                + index + " for partition " + part);
            multiDomain[level][currentPartition] = part.partitionStart;
            multiRange[level][currentPartition] = function.computeAggregate(
                this, startIndex, index, level - 1, 0, startIndex, index);
            startIndex = index;
  ...
  public interface XYPartitioner {
    ...
    public void getPartition(int partitionNumber, XYMultiresolution2 xy,
        double intervalStart, double intervalEnd, Partition result);

  }

  public interface XYAggregateFunction {
    /**
     * This function is given data, a previous (lower level) interval, the
     * requested interval indices, and is expected to return an aggregate
     * function of the given interval. The previous interval is given in case
     * computations can be reused. For example, if you are computing a partition
     * which spans a month, and you have previously computed 4 week partitions,
     * then you may be able to perform a linear combination of the previous
     * aggregate value plus some fraction of the remaining 0-3 days in the
     * month.
     * 
     * @param data
     ...
     * @return
     */
    double computeAggregate(XYMultiresolution2 data, int lowerStartInterval,
        int lowerEndInterval, int lowerLevel, int previousAggregate,
        int startInterval, int endInterval);
  }

  public static class DateQuantizedXYPartitionStrategy implements
      XYPartitionStrategy {

    public static final long SECOND = 1000, QUARTERMINUTE = SECOND * 15,
        HALFMINUTE = QUARTERMINUTE * 2, MINUTE = 60 * SECOND,
        QUARTERHOUR = MINUTE * 15, HALFHOUR = MINUTE * 30, HOUR = 60 * MINUTE,
        QUARTERDAY = HOUR * 6, HALFDAY = HOUR * 12, DAY = 24 * HOUR,
        WEEK = DAY * 7, HALFMONTH = DAY * 15, MONTH = 31 * DAY,
        QUARTER = MONTH * 3, BIANNUAL = QUARTER * 2, YEAR = DAY * 365,
        BIYEAR = YEAR * 2, FIVEYEAR = YEAR * 5, DECADE = YEAR * 5,
        QUARTERCENTURY = FIVEYEAR * 5, HALFCENTURY = QUARTERCENTURY * 2,
        CENTURY = DECADE * 10, QUARTERMILLENIUM = DECADE * 25,
        HALFMILLENIUM = QUARTERMILLENIUM * 2, MILLENIUM = CENTURY * 10;
    ...
    public static class DateQuantizedXYPartitioner implements XYPartitioner {
      ...
      public int getNumPartitions(XYMultiresolution2 xy, double intervalStart,
          double intervalEnd) {
        if (interval < WEEK)
          return nonDateSpecificPartition((long) intervalStart,
              (long) intervalEnd);
        Date date, dateEnd;
        ...
        if (interval == WEEK) {
          quantizedBegin = date.getTime() - date.getHours() * HOUR
              - date.getMinutes() * MINUTE - date.getTimezoneOffset() * MINUTE
              - date.getSeconds() * SECOND - date.getDay() * DAY
              - date.getTimezoneOffset() * HOUR;
          quantizedBegin = quantizedBegin - quantizedBegin % 1000;
          quantizedEnd = dateEnd.getTime() - dateEnd.getHours() * HOUR
              - dateEnd.getMinutes() * MINUTE - date.getTimezoneOffset()
              * MINUTE - dateEnd.getSeconds() * SECOND + (7 - dateEnd.getDay())
              * DAY - dateEnd.getTimezoneOffset() * HOUR;
          if (quantizedEnd < intervalEnd)
            quantizedEnd += WEEK;
      ...
      private int nonDateSpecificPartition(long intervalStart, long intervalEnd) {
        ...
        if (endRemain > 0)
          endRemain = interval - endRemain - 1;
        long startRemain = intervalStart % interval;
        if (startRemain > 0)
          startRemain = interval - startRemain;
        System.out.println("QIntervalStart "
            + date(intervalStart - startRemain) + " QIntervalEnd "
            + date(intervalEnd + endRemain));
      ...
      public void getPartition(int partitionNumber, XYMultiresolution2 xy,
          double intervalStart, double intervalEnd, Partition result) {
        result.data = xy;
        result.partitionStart = quantizeTo(intervalStart) + interval
            * partitionNumber;
      ...
      private long quantizeTo(double domainVal) {
        ...
        return date.getTime() - date.getHours() * HOUR - date.getMinutes()
            * MINUTE - date.getTimezoneOffset() * MINUTE - date.getSeconds()
            * SECOND - date.getDay() * DAY;
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < domain.length; i++) {
      ...
      start += Math.random()
          * XYMultiresolution2.DateQuantizedXYPartitionStrategy.HOUR;
    }
    ...
    xy.compute(new XYMultiresolution2.DateQuantizedXYPartitionStrategy(),
        new XYAggregateFunction() {
          public double computeAggregate(XYMultiresolution2 data,
              int lowerStartInterval, int lowerEndInterval, int lowerLevel,
              int previousAggregate, int startInterval, int endInterval) {
            double sum = 0;

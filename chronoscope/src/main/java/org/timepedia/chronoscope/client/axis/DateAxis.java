...
public class DateAxis extends ValueAxis {
    ...
    public double getHeight() {
        if (getOrientation() == AxisPanel.HORIZONTAL_AXIS) {
            ...
            return plot.getPlotBounds().height;
        }
    }
    ...
    public void init() {
        ...
        axisLabelHeight = renderer.isAxisLabelVisible() ? renderer.getLabelHeight(getChart().getView(),
                                                                                  getOrientation() == AxisPanel
                                                                                          .HORIZONTAL_AXIS ? "(Time)" :
                                                                                          "X") *
                ( getOrientation() == AxisPanel.HORIZONTAL_AXIS ? 1 : "(Time)".length() ) : 0;

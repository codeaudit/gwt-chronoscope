...
public class DomainAxisRenderer implements AxisRenderer, GssElement {
    ...
    private static final String[] monthLabels =
    ...
    private static final TickLabelFormatter rootFormatter = new RootTickLabelFormatter();
    ...
    public void init(View view) {
        if (axisProperties == null) {
            ...
            creditsWidth = view.getCanvas().stringWidth(CREDITS, CREDITS_FONT, CREDITS_WEIGHT, CREDITS_SIZE);
            creditsHeight = view.getCanvas().stringHeight(CREDITS, CREDITS_FONT, CREDITS_WEIGHT, CREDITS_SIZE);
    ...
    public void drawAxis(XYPlot plot, Layer layer, Bounds bounds, boolean gridOnly) {
        ...
        if (tlf.getSuperIntervalFormatter() == null) {
            ...
            drawInterval(plot, layer, bounds, tlf, int1, int1 + tlf.getInterval(), true);
            if (( dO + cd ) - int1 > tlf.getInterval()) {
                drawInterval(plot, layer, bounds, tlf, int2, int2 + tlf.getInterval(), true);
    ...
    private void drawInterval(XYPlot plot, Layer layer, Bounds bounds, TickLabelFormatter tlf, double intervalStart,
                              double intervalEnd, boolean isRoot) {
        ...
        double superLabelWidth = tlf.getSuperIntervalFormatter() == null ? 0 : tlf.getSuperIntervalFormatter()
        ...
        int numTicks = tlf.quantizeTicks(Math.min(emptySpace / ( labelWidth + 20 ), maxTicks));
        ...
        int i = isRoot ? 0 : 1;
        int loop = tlf.getSuperIntervalFormatter() == null ? numTicks + 1 : numTicks;
        for (; i < loop; i++) {
            double tickPos = tlf.getTick(intervalStart, intervalEnd, i, numTicks, maxTicks);

            drawTick(plot, layer, tickPos, bounds, tlf);
            if (i <= numTicks) {
                drawInterval(plot, layer, bounds, tlf.getSubIntervalFormatter(), tickPos, tlf.getTick(intervalStart,
                                                                                                      intervalEnd,
                                                                                                      i + 1, numTicks,
                                                                                                      maxTicks), false);
            }
    ...
    private TickLabelFormatter findInterval(TickLabelFormatter tlf, double dO, double cd) {
        if (tlf == null) {
            return null;
        }
    ...
    private void drawAxisLabel(Layer layer, Bounds bounds) {
        ...
        layer.drawText(center - halfLabelWidth, bounds.y + axis.getMaxLabelHeight() + 5, TIME_LABEL,
                       labelProperties.fontFamily, labelProperties.fontWeight, labelProperties.fontSize, textLayerName);
        // only show if enabled and a collision with the axis label is avoided
        if (Chronoscope.isShowCreditsEnabled() && center + halfLabelWidth < bounds.x + bounds.width - creditsWidth) {
            layer.save();
            layer.setTransparency(0.2f);
            layer.drawText(bounds.x + bounds.width - creditsWidth, bounds.y + bounds.height - creditsHeight, CREDITS,
                           CREDITS_FONT, CREDITS_WEIGHT, CREDITS_SIZE, textLayerName);
            layer.restore();
        }
    ...
    private void drawTick(XYPlot plot, Layer layer, double tickLocation, Bounds bounds, TickLabelFormatter tlf) {
        ...
        if (ux >= bounds.x && ux <= bounds.x + bounds.width) {
            ...
            if (gridProperties.visible) {
                ...
                player.fillRect(ux - bounds.x, 0, gridProperties.lineThickness, plot.getPlotBounds().height);
                player.restore();
            }
    ...
    private void drawLabel(XYPlot plot, Layer layer, double tickLoc, Bounds bounds, TickLabelFormatter tlf) {
...
//            return;
        ...
        layer.drawText(ux - labelWidth / 2, bounds.y + 5, label, axisProperties.fontFamily, axisProperties.fontWeight,
    ...
    private void drawHorizontalLine(Layer layer, double origin, double end, Bounds bounds) {
    ...
    private static class TenthsTickLabelFormatter extends AbstractTickLabelFormatter implements TickLabelFormatter {
        ...
        public String formatRelativeTick(double domainPoint) {
            ...
            return pad(d.getHours()) + ":" + pad(d.getMinutes()) + ":" + pad(d.getSeconds()) + "." + pad(
                    (int) ( d.getTime() / 100 % 10 ));
        }

        public String formatFullTick(double domainPoint) {
            Date d = new Date((long) domainPoint);
            return pad(d.getDate()) + " " + monthLabels[d.getMonth()] + "'" + ( d.getYear() + 1900 ) + " " + pad(
        ...
        public double getTick(double origin, double intervalEnd, int tickNum, int numTicks, int maxTicks) {
    ...
    private static class SecondsTickLabelFormatter extends AbstractTickLabelFormatter implements TickLabelFormatter {
        private final TickLabelFormatter superFormatter;
        ...
        public double quantizeToNearest(double dO) {
            Date d = new Date((long) dO);
            return new Date(d.getYear(), d.getMonth(), d.getDate(), d.getHours(), d.getMinutes(), 0).getTime();
    ...
    private static class MinutesTickLabelFormatter extends AbstractTickLabelFormatter implements TickLabelFormatter {
        private final TickLabelFormatter superFormatter;
        ...
        public double quantizeToNearest(double dO) {
            Date d = new Date((long) dO);
            return new Date(d.getYear(), d.getMonth(), d.getDate(), d.getHours(), 0, 0).getTime();
    ...
    private static class HoursTickLabelFormatter extends AbstractTickLabelFormatter implements TickLabelFormatter {
        private final TickLabelFormatter superFormatter;
    ...
    private static class DaysTickLabelFormatter extends AbstractTickLabelFormatter implements TickLabelFormatter {
        private final TickLabelFormatter superFormatter;
        ...
        public String formatFullTick(double domainPoint) {
            Date d = new Date((long) domainPoint);
            return pad(d.getDate()) + " " + monthLabels[d.getMonth()] + "'" + ( d.getYear() + 1900 );
        }
    ...
    private static class MonthsTickLabelFormatter extends AbstractTickLabelFormatter implements TickLabelFormatter {
    ...
    private static class RootTickLabelFormatter extends AbstractTickLabelFormatter implements TickLabelFormatter {

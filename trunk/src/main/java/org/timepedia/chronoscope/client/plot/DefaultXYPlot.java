...
//        else if(destinationOrigin + destDom > getDomainMax()) d = getDomainMax() - destDom;
        ...
        final PortableTimer t = view.createTimer(new PortableTimerTask() {
            double z = 1.0;
            final double zf = destDom / currentDomain;
            final double scale = Math.exp(Math.log(zf) / getNumAnimationFrames());
            ...
            final double translate = ( destCenter - domainCenter ) / getNumAnimationFrames();
            double frames = 1;
            ...
            public void run(PortableTimer t) {
                isAnimating = true;
                setCurrentDomain(getCurrentDomain() * scale);
                domainCenter += translate;
                domainOrigin = domainCenter - currentDomain / 2;
                redraw();

                if (frames++ < getNumAnimationFrames()) {
                    t.schedule(10);
                } else {

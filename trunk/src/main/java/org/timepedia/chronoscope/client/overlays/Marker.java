...
public class Marker implements Overlay, GssElement, Exportable {
    ...
    public void draw(Layer backingCanvas, String layer) {
        if (domainX <= plot.getDomainOrigin() || domainX >= plot.getDomainOrigin() + plot.getCurrentDomain()) {
            return;
        }

        int point = plot.getNearestVisiblePoint(domainX, seriesNum == -1 ? 0 : seriesNum) - 1;
        ...
        if (width == -1) {
            ...
            height = backingCanvas.stringHeight(label, "Verdana", "normal", "9pt") + 2;
        ...
        backingCanvas.drawText(x, y, label, markerProperties.fontFamily, markerProperties.fontWeight,
                               markerProperties.fontSize, layer);
        backingCanvas.restore();
    }

    private double drawOval(Layer backingCanvas, double x, double y, double yp, int dir) {
        ...
        double startAngle = -Math.PI - Math.PI / 4 - Math.PI / 2 - Math.PI / 8;
        double endAngle = Math.PI + Math.PI / 4 - Math.PI / 2 + Math.PI / 8;
        if (dir == 0) {
            startAngle = -Math.PI / 2 - Math.PI / 4 - Math.PI / 8;
            endAngle = -Math.PI / 2 + Math.PI / 4 + Math.PI / 8;
        }

        backingCanvas.arc(x + width / 2, y + height / 2, width + 1, startAngle, endAngle, 1);
    ...
    public boolean isHit(int x, int y) {
        ...
        if (width == -1) {
            width = view.getCanvas().stringWidth(label, "Verdana", "normal", "9pt");
            height = view.getCanvas().stringHeight(label, "Verdana", "normal", "9pt") + 2;

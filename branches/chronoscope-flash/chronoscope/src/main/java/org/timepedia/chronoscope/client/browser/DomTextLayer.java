...
/**
 * A class that implements text rendering by positioning DIVs with text or images over the canvas.
 *
...
public abstract class DomTextLayer extends AbstractLayer {
    ...
    public void drawRotatedText(final double x, final double y, final double angle, final String label,
                                String fontFamily, String fontWeight, String fontSize, final String layerName,
                                final Chart chart) {
        String metricsKey = getStrokeColor() + fontFamily + fontWeight + fontSize + angle;
        CanvasFontMetrics rmt = (CanvasFontMetrics) fontMetricsCache.get(metricsKey);
        ...
        if (rmt.rfm == null) {
            ...
            FontRendererServiceAsync fontRendererServiceAsync = FontRendererService.FontRenderServiceUtil.getInstance();
            ...
            fontRendererServiceAsync.getRenderedFontMetrics(fontFamily, fontWeight, ifs + "pt", getStrokeColor(),
                                                            (float) angle, new AsyncCallback() {
    ...
    public int rotatedStringWidth(String str, double rotationAngle, String fontFamily, String fontWeight,
                                  String fontSize) {
        String metricsKey = getStrokeColor() + fontFamily + fontWeight + fontSize + rotationAngle;
        CanvasFontMetrics rmt = (CanvasFontMetrics) fontMetricsCache.get(metricsKey);
        if (rmt != null && rmt.rfm != null) {
            return rmt.rfm.stringWidth(str.toCharArray());
        } else {
            return super.rotatedStringWidth(str, rotationAngle, fontFamily, fontWeight, fontSize);
        }
    }
    ...
    public int rotatedStringHeight(String str, double rotationAngle, String fontFamily, String fontWeight,
        ...
        if (rmt != null && rmt.rfm != null) {
            ...
            return super.rotatedStringHeight(str, rotationAngle, fontFamily, fontWeight, fontSize);
        }
    }

    private void drawTextImage(double x, double y, String label, String layerName, double angle,
                               RenderedFontMetrics rfm) {
        TextLayer layer = getTextLayer(layerName);
        Element layerElem = layer.layerElem;
        double x1 = x, y1 = y;
        int sign = 1;

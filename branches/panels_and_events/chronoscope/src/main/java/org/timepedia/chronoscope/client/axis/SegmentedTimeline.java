...
public class SegmentedTimeline implements Timeline, Cloneable, Serializable {
    ...
    static {
        ...
        // current locale
        Date d = new Date(0, 0, 1, 0, 0, 0);
        int dow = d.getDay();
       
        while(dow != 1) {
          d = new Date(d.getTime() + 86400l*1000l);
          dow = d.getDay();
        }
        
        FIRST_MONDAY_AFTER_1900 = d.getTime();
    }
     ...
     */
    public SegmentedTimeline(double segmentSize,
  ...
  public SegmentedTimeline() {
     ...
     */
    public void setStartTime(double millisecond) {
     ...
     */
    public double toTimelineValue(double millisecond) {
  
        double result;
        double rawMilliseconds = millisecond - this.startTime;
        double groupMilliseconds = rawMilliseconds % this.segmentsGroupSize;
        int groupIndex = (int) Math.floor (rawMilliseconds / this.segmentsGroupSize);
        ...
        if (groupMilliseconds >= this.segmentsIncludedSize) {
            ...
            if (segment.inExceptionSegments()) {
                ...
                double shiftedSegmentedValue = millisecond - this.startTime;
                double x = shiftedSegmentedValue % this.segmentsGroupSize;
                double y = Math.floor(shiftedSegmentedValue / this.segmentsGroupSize);

                double wholeExceptionsBeforeDomainValue =
                    getExceptionSegmentCount(this.startTime, millisecond - 1);

//                double partialTimeInException = 0;
...
//                }
     ...
     */
    public double toMillisecond(double timelineValue) {
        
        // calculate the result as if no exceptions
        Segment result = new Segment(this.startTime + timelineValue 
            + (Math.floor(timelineValue / this.segmentsIncludedSize)) 
            * this.segmentsExcludedSize);
        
        double lastIndex = this.startTime;
        ...
        while (lastIndex <= result.segmentStart) {
            ...
            double exceptionSegmentCount;
            while ((exceptionSegmentCount = getExceptionSegmentCount(
                 lastIndex, Math.floor(result.millisecond / this.segmentSize) 
        ...
        return getTimeFromdouble(result.millisecond); 
    }
     ...
     * @return The milliseconds.
     */
    public double getTimeFromdouble(double date) {
        double result = date;
     ...
     */
    public boolean containsDomainValue(double millisecond) {
        Segment segment = getSegment(millisecond);
     ...
     */
    public boolean containsDomainRange(double domainValueStart, 
                                       double domainValueEnd) {
     ...
     */
    public void addException(double millisecond) {
     ...
     */
    public void addException(double fromDomainValue, double toDomainValue) {
     ...
     */
    public void addBaseTimelineException(double domainValue) {
        ...
        if (baseSegment.inIncludeSegments()) {
            ...
            while (segment.getSegmentStart() <= baseSegment.getSegmentEnd()) {
                if (segment.inIncludeSegments()) {
                    ...
                    double fromDomainValue = segment.getSegmentStart();
                    double toDomainValue;
                    do {
     ...
     */
    public void addBaseTimelineExclusions(double fromBaseDomainValue, 
                                          double toBaseDomainValue) {
        ...
        while (baseSegment.getSegmentStart() <= toBaseDomainValue) {
            ...
            double baseExclusionRangeEnd = baseSegment.getSegmentStart() 
     ...
     */
    public double getExceptionSegmentCount(double fromMillisecond, 
     ...
     */
    public Segment getSegment(double millisecond) {
     ...
     *
     * @param date Date to convert to double.
     * 
     * @return The milliseconds.
     */
     ...
     * @return The date.
     */
    public Date getDate(double value) {
    ...
    public class Segment implements Comparable, Cloneable, Serializable {
         ...
         * @param millisecond  the millisecond (as encoded by java.util.Date).
         */
        protected Segment(double millisecond) {
      ...
      /**
         ...
         */
        public int calculateSegmentNumber(double millis) {
            if (millis >= SegmentedTimeline.this.startTime) {
                return (int) Math.floor((millis - SegmentedTimeline.this.startTime) 
         ...
         *         segment.
         */
        public boolean contains(double from, double to) {
         ...
         *         interval.
         */
        public boolean contained(double from, double to) {
         ...
         * @return A segment.
         */
        public Segment intersect(double from, double to) {
        ...
        public void inc(double n) {
            ...
            double m = n * SegmentedTimeline.this.segmentSize;
            this.segmentStart += m;
            this.segmentEnd += m;
            this.millisecond += m;
        }
    ...
    protected class SegmentRange extends Segment { 
         ...
         */
        public SegmentRange(double fromMillisecond, double toMillisecond) {
...
//            }
      ...
      /**
        ...
        public Segment intersect(double from, double to) {
            ...
            // toSegment.dec();
            double start = Math.max(from, this.segmentStart);
            double end = Math.min(to, this.segmentEnd);
            // double start = Math.max(
            //     fromSegment.getSegmentStart(), this.segmentStart
            // );
            // double end = Math.min(toSegment.getSegmentEnd(), this.segmentEnd);
            if (start <= end) {
    ...
    protected class BaseTimelineSegmentRange extends SegmentRange {
         ...
         */
        public BaseTimelineSegmentRange(double fromDomainValue, 
                                        double toDomainValue) {
